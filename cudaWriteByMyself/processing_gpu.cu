...
*/
 ...
 { 
    ...
    rdComplexMultiply<<<block,grid>>>(d_signal, d_ori, M, N);                                              //乘以共轭直, 直接改的d_signal
    cufftHandle plan3;
 ...
 float doGpuProcessing(cuDoubleComplex* signal, cuDoubleComplex* ori, int M, int N)
{
     ...
     /*int inembed[1] = {0};
     int onembed[1] = { 0 };
     int number_N[1] = { N };                                         //这不跟longint冲突了
     int istride = 1;
     int rank = 1;
     int ostride = 1;
     cufftHandle plan1;
     ...
     rdComplexMultiply << <block1, grid1 >> > (d_signal, d_ori, M, N);                                              //乘以共轭直, 直接改的d_signal
     //cufftHandle plan3;
     //cufftPlan1d(&plan3, N, CUFFT_Z2Z, M);
     ...
     */
     ...
     cuDoubleComplex* dd_signal;
     cudaMalloc((void**)&dd_signal, memSize);
     rdComplexTranspose << <block2, grid2 >> > (dd_signal, d_signal, M, N);                                    //先转置好做列的fft           //这个得狠狠的优化
     cufftHandle plan3;
     cufftPlan1d(&plan3, M, CUFFT_Z2Z, N);                                                                                                          //按理说fft点数应该大于M来着 即一般k>M
     cufftExecZ2Z(plan3, (cufftDoubleComplex*)dd_signal, (cufftDoubleComplex*)dd_signal, CUFFT_FORWARD);                                                 //做fft
     cufftDestroy(plan3);
     rdComplexTranspose << <block2, grid2 >> > (d_signal, dd_signal, N, M);                                 //转置回去的时候是N列M行, 所以是N,M!!!!!
     cudaFree(dd_signal);
     /*int inembed[1] = {0};
     int onembed[1] = { 0 };
     int number_M[1] = { M };                                         
     ...
     cudaMemset(d_out, 0, memSizeDouble);                                 //咱也不知道有没有意义这一步         有的, 不然边缘的就没赋值了
     dim3 block3, grid3;
     block3.x = BLOCKX;
