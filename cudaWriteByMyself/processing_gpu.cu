...
*/
 ...
 void mtd(cuDoubleComplex* d_signal, int M, int N, dim3 grid1, dim3 block1, dim3 grid2, dim3 block2)
 {
     ...
     rdComplexTranspose <<< grid1, block1 >>> (dd_signal, d_signal, M, N);                                    //先转置好做列的fft           //这个得狠狠的优化
     ...
     cufftDestroy(plan);
     rdComplexTranspose <<< grid2, block2 >>> (d_signal, dd_signal, N, M);                                 //转置回去的时候是N列M行, 所以是N,M!!!!!
     cudaFree(dd_signal);
 }
 ...
 float doGpuProcessing(cuDoubleComplex* signal, cuDoubleComplex* ori, int M, int N, dim3* grid, dim3* block)
{
     ...
     cufftDestroy(plan3);
     ////////////////////////////////////////////////////////////////////////////////
     mtd(d_signal, M, N, grid[1], block[1], grid[2], block[2]);                                                               //脉冲压缩的结果送到mtd
     //test(d_signal, M, N);
     double* d_sqSignal;
 ...
 float goodTransDoGpuProcessing(cuDoubleComplex* signal, cuDoubleComplex* ori, int M, int N, dim3* grid, dim3* block)
 {
     ...
     cufftDestroy(plan3);
     ////////////////////////////////////////////////////////////////////////////////
     betterMtd(d_signal, M, N, grid[1], block[1], grid[2], block[2]);                                                               //脉冲压缩的结果送到mtd
     //test(d_signal, M, N);
     double* d_sqSignal;
     cudaMalloc((void**)&d_sqSignal, memSize);
     double* d_out;
